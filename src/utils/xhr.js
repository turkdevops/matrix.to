...
function addCacheBuster(urlStr, random = Math.random) {
  ...
  return urlStr +
         `_cacheBuster=${Math.ceil(random() * Number.MAX_SAFE_INTEGER)}`;
}
...
function xhrAsPromise(xhr, method, url) {
  return new Promise((resolve, reject) => {
    ...
    xhr.addEventListener(
        "error", () => reject(new ConnectionError(`Error ${method} ${url}`)));
    xhr.addEventListener("timeout", () => reject(new ConnectionError(
                                        `Timeout ${method} ${url}`, true)));
  });
}
