...
export class TemplateView {
  ...
  _updateFromValue(changedProps) { this.update(this._value, changedProps); }
...
class TemplateBuilder {
  constructor(templateView) { this._templateView = templateView; }

  get _value() { return this._templateView._value; }
  ...
  _addClassNamesBinding(node, obj) {
    this._addAttributeBinding(node, "className",
                              value => classNames(obj, value));
  }
  ...
  // sugar
  createTemplate(render) { return vm => new TemplateView(vm, render); }
  ...
  if(fn, viewCreator) {
    return this.mapView(value => !!fn(value),
...
for (const [ns, tags] of Object.entries(TAG_NAMES)) {
  for (const tag of tags) {
    TemplateBuilder.prototype[tag] = function(
        attributes,
        children) { return this.elNS(ns, tag, attributes, children); };
  }
}
