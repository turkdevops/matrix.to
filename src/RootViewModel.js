...
export class RootViewModel extends ViewModel {
    ...
    _updateChildVMs(oldLink) {
        if (this.link) {
            ...
            if (!oldLink || !oldLink.equals(this.link)) {
                this.openLinkViewModel = new OpenLinkViewModel(
                    this.childOptions({
                        link: this.link,
            ...
            this.openLinkViewModel = null;
            this.createLinkViewModel = new CreateLinkViewModel(
                this.childOptions()
            );
        }
        this.emitChange();
    ...
    updateHash(hash) {
        if (hash.startsWith("#/policy/")) {
            ...
            this.loadServerPolicyViewModel = new LoadServerPolicyViewModel(
                this.childOptions({ server })
            );
