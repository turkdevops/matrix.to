...
export class ClientViewModel extends ViewModel {
    ...
    _update() {
        const matchingPlatforms = getMatchingPlatforms(
            this._client,
            this.platforms
        );
        this._webPlatform = matchingPlatforms.find((p) => isWebPlatform(p));
        this._nativePlatform = matchingPlatforms.find((p) => !isWebPlatform(p));
        const preferredPlatform = matchingPlatforms.find(
            (p) => p === this.preferences.platform
        );
    ...
    _createOpenActions() {
        ...
        if (hasPreferredWebInstance) {
            if (this._proposedPlatform === this._nativePlatform) {
                ...
                deepLinkLabel = `Open on ${this._client.getPreferredWebInstance(
                    this._link
                )}`;
            }
        }
        const actions = [];
        const proposedDeepLink = this._client.getDeepLink(
            this._proposedPlatform,
            this._link
        );
        if (proposedDeepLink) {
        ...
        if (
            hasPreferredWebInstance &&
            this._webPlatform &&
            this._proposedPlatform !== this._webPlatform
        ) {
            actions.push({
                label: `Open on ${this._client.getPreferredWebInstance(
                    this._link
                )}`,
    ...
    _createInstallActions() {
        ...
        if (this._webPlatform) {
            const webDeepLink = this._client.getDeepLink(
                this._webPlatform,
                this._link
            );
            if (webDeepLink) {
                const webLabel = this.hasPreferredWebInstance
                    ? `Open on ${this._client.getPreferredWebInstance(
                          this._link
                      )}`
    ...
    get hostedByBannerLabel() {
        const preferredWebInstance = this._client.getPreferredWebInstance(
            this._link
        );
        if (this._webPlatform && preferredWebInstance) {
            ...
            if (subDomainIdx !== -1) {
                label = preferredWebInstance.substr(
                    preferredWebInstance.length - subDomainIdx + 1
                );
            }
    ...
    get textInstructions() {
        let instructions = this._client.getLinkInstructions(
            this._proposedPlatform,
            this._link
        );
