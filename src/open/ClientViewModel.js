...
import {isDesktopPlatform, isWebPlatform, Platform} from "../Platform.js";
...
function getMatchingPlatforms(client, supportedPlatforms) {
  ...
  const matchingPlatforms =
      supportedPlatforms.filter(p => { return clientPlatforms.includes(p); });
...
export class ClientViewModel extends ViewModel {
  ...
  _update() {
    const matchingPlatforms =
        getMatchingPlatforms(this._client, this.platforms);
    ...
    const preferredPlatform =
        matchingPlatforms.find(p => p === this.preferences.platform);
    this._proposedPlatform =
        preferredPlatform || this._nativePlatform || webPlatform;

    this.openActions = this._createOpenActions();
    this.installActions = this._createInstallActions();
    this._clientCanIntercept =
        !!(this._nativePlatform &&
           this._client.canInterceptMatrixToLinks(this._nativePlatform));
  ...
  _createOpenActions() {
    ...
    if (proposedDeepLink) {
      actions.push({
        ...
        activated : () => {
          ...
          if (this._showOpen &&
              this._proposedPlatform === this._nativePlatform) {
    ...
    // show only if there is a preferred instance, and if we don't already link
    // to it in the first button
    if (hasPreferredWebInstance && this._webPlatform &&
        this._proposedPlatform !== this._webPlatform) {
      actions.push({
        label : `Open on ${this._client.getPreferredWebInstance(this._link)}`,
        url : this._client.getDeepLink(this._webPlatform, this._link),
        kind : "open-in-web",
        activated : () => {} // don't persist this choice as we don't persist
                             // the preferred web instance, it's in the url
      });
    }
    return actions;
  }
  ...
  _createInstallActions() {
    ...
    if (this._nativePlatform) {
      const nativeActions =
          (this._client.getInstallLinks(this._nativePlatform) ||
           []).map(installLink => {
            return {
              ...
              primary : true,
              activated : () => this.preferences.setClient(
                  this._client.id, this._nativePlatform),
            };
          });
    ...
    if (this._webPlatform) {
      const webDeepLink =
          this._client.getDeepLink(this._webPlatform, this._link);
      if (webDeepLink) {
        const webLabel =
            this.hasPreferredWebInstance
                ? `Open on ${this._client.getPreferredWebInstance(this._link)}`
  ...
  get hasPreferredWebInstance() {
    // also check there is a web platform that matches the platforms the user is
    // on (mobile or desktop web)
    return this._webPlatform &&
           typeof this._client.getPreferredWebInstance(this._link) === "string";
  }

  get hostedByBannerLabel() {
    const preferredWebInstance =
        this._client.getPreferredWebInstance(this._link);
    if (this._webPlatform && preferredWebInstance) {
      let label = preferredWebInstance;
      const subDomainIdx = preferredWebInstance.lastIndexOf(
          ".", preferredWebInstance.lastIndexOf("."));
      if (subDomainIdx !== -1) {
        label = preferredWebInstance.substr(preferredWebInstance.length -
                                            subDomainIdx + 1);
      }
  ...
  get homepage() { return this._client.homepage; }

  get identifier() { return this._link.identifier; }

  get description() { return this._client.description; }

  get clientId() { return this._client.id; }

  get name() { return this._client.name; }

  get iconUrl() { return this._client.icon; }

  get stage() { return this._showOpen ? "open" : "install"; }
  ...
  get showDeepLinkInInstall() {
    // we can assume this._nativePlatform as this._clientCanIntercept already
    // checks it
    return this._clientCanIntercept &&
           !!this._client.getDeepLink(this._nativePlatform, this._link);
  }
  ...
  get showBack() { return !!this._clientListViewModel; }
  ...
  back() {
    if (this._clientListViewModel) {
      ...
      // in the list with all clients, and also if we refresh, we get the list
      // with all clients rather than having our "change client" click reverted.
