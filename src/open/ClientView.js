...
import {tag, text} from "../utils/html.js";
...
export class ClientView extends TemplateView {
  ...
  render(t, vm) {
    return t.div({
      className : {
        "ClientView" : true,
        "isPreferred" : vm => vm.hasPreferredWebInstance
      }
    },
                 [
                   ...vm.hasPreferredWebInstance
                       ? [ t.div({className : "hostedBanner"},
                                 vm.hostedByBannerLabel) ]
                       : [],
                         ...
                         [
                           t.div({className : "description"},
                                 [
                                   ...
                                   t.p({className : "platforms"},
                                       formatPlatforms(
                                           vm.availableOnPlatformNames)),
                                 ]),
                           t.img({className : "clientIcon", src : vm.iconUrl})
                         ]),
                   t.mapView(vm => vm.stage,
                             stage => {
                               switch (stage) {
                               case "open":
                                 return new OpenClientView(vm);
                               case "install":
                                 return new InstallClientView(vm);
                               }
                             }),
                 ]);
  }
      ...
      const copyButton = t.button({
        ...
        onClick : evt => {
          if (copy(vm.copyString, copyButton.parentElement)) {
            ...
            setTimeout(() => { copyButton.className = "copy"; }, 2000);
          }
        }
      });
      children.push(
          t.p({className : "instructions"},
              renderInstructions(textInstructions).concat(copyButton)));
    }

    const actions = t.div({className : "actions"},
                          vm.installActions.map(a => renderAction(t, a)));
    ...
    if (vm.showDeepLinkInInstall) {
      const openItHere = t.a({
        ...
        onClick : () => vm.openActions[0].activated(),
                             ...
                             "open it here");
      children.push(t.p([
        `If you already have ${vm.name} installed, you can `, openItHere, "."
      ]))
    }
...
function showBack(t, vm) {
  return t.p(
      {
        className : {caption : true, "back" : true, hidden : vm => !vm.showBack}
      },
      [
...
function renderAction(t, a) {
  ...
  switch (a.kind) {
  case "play-store":
    badgeUrl = "images/google-play-us.svg";
    break;
  case "fdroid":
    badgeUrl = "images/fdroid-badge.png";
    break;
  case "apple-app-store":
    badgeUrl = "images/app-store-us-alt.svg";
    break;
  case "flathub":
    badgeUrl = "images/flathub-badge.svg";
    break;
  }
  return t.a({
    href : a.url,
