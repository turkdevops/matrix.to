...
export class ServerConsentView extends TemplateView {
    render(t, vm) {
        const useAnotherServer = t.button(
            { className: "text", onClick: () => vm.setShowServers() },
            "use another server"
        );
        const continueWithoutPreview = t.button(
            {
                className: "text",
                onClick: () =>
                    vm.continueWithoutConsent(this._askEveryTimeChecked),
            },
...
class ServerOptions extends TemplateView {
    ...
    _onClickOther(evt) {
        ...
        const radio = Array.from(textField.form.elements.selectedServer).find(
            (r) => r.value === "other"
        );
        if (!radio.checked) {
            radio.checked = true;
