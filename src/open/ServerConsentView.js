...
export class ServerConsentView extends TemplateView {
  render(t, vm) {
    const useAnotherServer =
        t.button({className : "text", onClick : () => vm.setShowServers()},
                 "use another server");
    ...
    return t.div({className : "ServerConsentView"}, [
      t.p([
        ...
        t.strong(vm => vm.selectedServer || "…"), " homeserver ",
        ...
        " or ", continueWithoutPreview, "."
      ]),
      t.form({
        action : "#",
        id : "serverConsentForm",
        onSubmit : evt => this._onSubmit(evt)
      },
             [
               t.mapView(vm => vm.showSelectServer,
                         show => show ? new ServerOptions(vm) : null),
               t.div({className : "actions"},
                     [
                       t.label([
                         t.input({type : "checkbox", name : "askEveryTime"}),
                         "Ask every time"
                       ]),
                       t.input({
                         type : "submit",
...
class ServerOptions extends TemplateView {
  render(t, vm) {
    const options = vm.servers.map(server => {return t.div(t.label([
                                     t.input({
                                       type : "radio",
                                       name : "selectedServer",
                                       value : server,
  ...
  _onClickOther(evt) {
    ...
    const radio = Array.from(textField.form.elements.selectedServer)
