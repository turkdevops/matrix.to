            ...
            t.mapView(
                ...
                () => {
                    return new TemplateView(vm, (t) => {
                        return t.div(
                            { className: "list" },
                            vm.clientList.map((clientViewModel) => {
            ...
            t.div(
                t.label([
                    t.input({
                        ...
                        onChange: (evt) =>
                            (vm.showUnsupportedPlatforms = evt.target.checked),
                    }),
                    "Show apps not available on my platform",
                ])
            ),
            t.div(
                t.label({ className: "filterOption" }, [
                    t.input({
                        type: "checkbox",
                        checked: vm.showExperimental,
                        onChange: (evt) =>
                            (vm.showExperimental = evt.target.checked),
                    }),
...
class ContinueWithClientView extends TemplateView {
    render(t, vm) {
        return t.div({ className: "ClientListView" }, [
            t.div(
                { className: "list" },
                t.view(new ClientView(vm.clientViewModel))
            ),
        ]);
    }
}
