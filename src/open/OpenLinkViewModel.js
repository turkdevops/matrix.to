...
export class OpenLinkViewModel extends ViewModel {
  ...
  async _showLink() {
    ...
    const preferredClient =
        clientId ? this._clients.find(c => c.id === clientId) : null;
    ...
    this.previewViewModel = new PreviewViewModel(this.childOptions(
        {link : this._link, consentedServers : this.preferences.homeservers}));
  ...
  get previewDomain() { return this.previewViewModel?.domain; }

  get previewFailed() { return this.previewViewModel?.failed; }

  get showClientsLabel() { return getLabelForLinkKind(this._link.kind); }
