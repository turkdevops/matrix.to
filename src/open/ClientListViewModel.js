...
export class ClientListViewModel extends ViewModel {
  ...
  get showUnsupportedPlatforms() { return this._showUnsupportedPlatforms; }

  get showExperimental() { return this._showExperimental; }
  ...
  _filterClients() {
    const clientVMs =
        this._clients
            .filter(client => {
              const platformMaturities =
                  this.platforms.map(p => client.getMaturity(p));
              const isStable = platformMaturities.includes(Maturity.Stable) ||
                               platformMaturities.includes(Maturity.Beta);
              const isSupported =
                  client.platforms.some(p => this.platforms.includes(p));
  ...
  _pickClient(client) {
    this.clientViewModel =
        this.clientList.find(vm => vm.clientId === client.id);
