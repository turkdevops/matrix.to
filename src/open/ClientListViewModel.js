...
export class ClientListViewModel extends ViewModel {
    ...
    _filterClients() {
        const clientVMs = this._clients
            .filter((client) => {
                const platformMaturities = this.platforms.map((p) =>
                    client.getMaturity(p)
                );
                const isStable =
                    platformMaturities.includes(Maturity.Stable) ||
                    platformMaturities.includes(Maturity.Beta);
                const isSupported = client.platforms.some((p) =>
                    this.platforms.includes(p)
                );
        ...
        const preferredClientVMs = clientVMs.filter(
            (c) => c.hasPreferredWebInstance
        );
        const otherClientVMs = clientVMs.filter(
            (c) => !c.hasPreferredWebInstance
        );
    ...
    _pickClient(client) {
        this.clientViewModel = this.clientList.find(
            (vm) => vm.clientId === client.id
        );
