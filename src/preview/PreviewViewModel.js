...
import { IdentifierKind, LinkKind } from "../Link.js";
...
export class PreviewViewModel extends ViewModel {
    ...
    async load() {
        ...
        if (supportsPreview) {
            ...
            for (const server of this._consentedServers) {
                try {
                    const homeserver = await resolveServer(
                        this.request,
                        server
                    );
                    switch (this._link.kind) {
                        case LinkKind.User:
                            await this._loadUserPreview(
                                homeserver,
                                this._link.identifier
                            );
                            break;
                        case LinkKind.Room:
                        case LinkKind.Event:
        ...
        if (this._consentedServers.length && supportsPreview) {
            this.domain = this._consentedServers[
                this._consentedServers.length - 1
            ];
            this.failed = true;
        }
        this.emitChange();
    ...
    get hasTopic() {
        return this._link.kind === LinkKind.Room;
    }
    get hasMemberCount() {
    ...
    async _loadRoomPreview(homeserver, link) {
        ...
        this.name =
            publicRoom?.name || publicRoom?.canonical_alias || link.identifier;
