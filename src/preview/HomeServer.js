...
export async function resolveServer(request, baseURL) {
  ...
  {
    const {status, body} =
        await request(`${baseURL}/.well-known/matrix/client`, {
          method : "GET"
        }).response();
    if (status === 200) {
...
export class HomeServer {
  ...
  async getUserProfile(userId) {
    const {body} = await this
                       ._request(`${this.baseURL}/_matrix/client/r0/profile/${
  ...
  async findPublicRoomById(roomId) {
    const {body, status} =
        await this
            ._request(`${this.baseURL}/_matrix/client/r0/directory/list/room/${
    ...
    do {
      ...
      const {body, status} =
          await this
              ._request(`${this.baseURL}/_matrix/client/r0/publicRooms?${
                  queryParams}`)
              .response();
  ...
  async getRoomIdFromAlias(alias) {
    const {status, body} =
        await this
            ._request(`${this.baseURL}/_matrix/client/r0/directory/room/${
  ...
  async getPrivacyPolicyUrl(lang = "en") {
    ...
    const {status, body} =
        await this
            ._request(`${this.baseURL}/_matrix/client/r0/register`, options)
            .response();
    if (status === 401 && body) { // Unauthorized
      const hasTermsStage =
          body.flows.some(flow => flow.stages.includes("m.login.terms"));
      if (hasTermsStage) {
        const privacyPolicy =
            body.params?.["m.login.terms"]?.policies?.privacy_policy;
        if (privacyPolicy) {
          const firstLang =
              Object.keys(privacyPolicy).find(k => k !== "version");
  ...
  mxcUrlThumbnail(url, width, height, method) {
    ...
    if (parts) {
      ...
      const httpUrl = `${this.baseURL}/_matrix/media/r0/thumbnail/${
