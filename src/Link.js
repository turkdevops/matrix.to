...
function asPrefix(identifierKind) {
  ...
  case IdentifierKind.UserId:
    return "@";
  default:
    throw new Error("invalid id kind " + identifierKind);
  }
}
...
function getWebInstanceMap(queryParams) {
  ...
  const postfix = "]";
  const webInstanceParams = queryParams.filter(
      ([ key ]) => key.startsWith(prefix) && key.endsWith(postfix));
...
export const LinkKind = createEnum("Room", "User", "Group", "Event")

export class Link {
  static validateIdentifier(identifier) {
    return !!(
        USERID_PATTERN.exec(identifier) || ROOMALIAS_PATTERN.exec(identifier) ||
        ROOMID_PATTERN.exec(identifier) || GROUPID_PATTERN.exec(identifier));
  }
  ...
  static parse(fragment) {
    ...
    if (queryParamsStr) {
      ...
      viaServers = queryParams.filter(([ key, value ]) => key === "via")
                       .map(([, value ]) => value);
    ...
    if (matches) {
      ...
      return new Link(clientId, viaServers, IdentifierKind.UserId, localPart,
                      server, webInstances);
    }
    ...
    if (matches) {
      const server = matches[2];
      const localPart = matches[1];
      return new Link(clientId, viaServers, IdentifierKind.RoomAlias, localPart,
                      server, webInstances, eventId);
    }
    matches = ROOMID_PATTERN.exec(identifier);
    if (matches) {
      const server = matches[2];
      const localPart = matches[1];
      return new Link(clientId, viaServers, IdentifierKind.RoomId, localPart,
                      server, webInstances, eventId);
    }
    ...
    if (matches) {
      const server = matches[2];
      const localPart = matches[1];
      return new Link(clientId, viaServers, IdentifierKind.GroupId, localPart,
                      server, webInstances);
    }
    return null;
  }

  constructor(clientId, viaServers, identifierKind, localPart, server,
              webInstances, eventId) {
  ...
  equals(link) {
    return link && link.identifier === this.identifier &&
           this.servers.length === link.servers.length &&
           this.servers.every((s, i) => link.servers[i] === s) &&
           Object.keys(this.webInstances).length ===
               Object.keys(link.webInstances).length &&
           Object.keys(this.webInstances)
               .every(k => this.webInstances[k] === link.webInstances[k]);
  }
